...
export class Game {
    playerName: string;
    bees: Bee[] = [];
    aliveBees: Record<string, number> = {};
    totalHealth: number = 0;

    constructor(playerName: string, bees: Bee[] = []) {
    ...
    initBees(queen = 1, worker = 5, drone = 8): void {
    ...
    */
    ...
    showAttackValue(bee: Bee): void {
        ...
        if (beeElement) {
            ...
            const containerRect = document.getElementById('bee-container')?.getBoundingClientRect();
            ...
            container?.appendChild(attackValueElement);
    ...
    getAttackValue(type: string): number {
        switch (type) {
            case 'queen':
                return 8;
            case 'worker':
    ...
    */
    updateUI(): void {
        this.updateBeeIcons();
        // Update bee counts
        (document.getElementById('queen-count') as HTMLElement).innerText = this.aliveBees['queen'].toString();
        (document.getElementById('worker-count') as HTMLElement).innerText = this.aliveBees['worker'].toString();
        (document.getElementById('drone-count') as HTMLElement).innerText = this.aliveBees['drone'].toString();
    ...
    renderBeesRandomly(): void {
        const container = document.getElementById('bee-container') as HTMLElement;
